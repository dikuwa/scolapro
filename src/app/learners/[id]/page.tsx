        ...
        <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            <div className="relative shrink-0">
              <span className="relative grid size-16 place-items-center overflow-hidden rounded-[var(--radius-md)] bg-brand-soft text-sm font-semibold text-brand-strong shadow-[var(--shadow-xs)]"><span aria-hidden="true">{avatarInitials}</span>{learner.photoUrl ? <img src={learner.photoUrl} alt={`Photo of ${learner.name}`} className="absolute inset-0 size-full object-cover" /> : null}</span>

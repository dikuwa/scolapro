import { Clock3, MailPlus, School, X } from "lucide-react";
import { redirect } from "next/navigation";
import { AppShell } from "@/components/shell/app-shell";
import { SchoolInvitationForm } from "@/features/platform/school-invitation-form";
import { revokeSchoolStaffInvitation } from "@/features/platform/server/actions";
...
export default async function SchoolInvitationsPage() {
  ...
  const { schoolOptions, invitations } = await getSchoolInvitationAdminData(membership.schoolId);
  const pendingCount = invitations.filter((item) => item.status === "pending").length;
  const schoolName = schoolOptions[0]?.name ?? membership.schoolName;
    ...
    <AppShell>
      <section>
        <div className="mb-6">
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">Invite teachers and staff who will work within this school. The school is fixed by your current administration context and cannot be changed from this workflow.</p>
        </div>
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(22rem,0.95fr)] xl:items-start">
          ...
          <section className="rounded-[var(--radius-md)] bg-surface-muted p-4 sm:p-5">
            ...
            <div className="mt-5"><SchoolInvitationForm schools={schoolOptions} mode="school" /></div>
          </section>
        </div>
      </section>
    </AppShell>

...
export default async function DataCorrectionsPage() {
  ...
  const membership = context.memberships.find((candidate) => reviewRoles.has(candidate.roleKey));
  ...
  return (
    <AppShell>
      <div className="space-y-5">
        <div>
          <h1 className="scolapro-page-title text-xl">Data corrections</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Review learner and guardian corrections proposed by staff. Authoritative records change only after approval.
          </p>
        </div>
        <ProfileChangeReviewList requests={requests} />
      </div>
    </AppShell>
  );
}

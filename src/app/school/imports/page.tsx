...
import { AlertTriangle, BookOpenCheck, CheckCircle2, FileSpreadsheet, HeartHandshake, Link2, SkipForward, Upload, UsersRound } from "lucide-react";
    ...
    <AppShell>
      <div className="space-y-5">
        ...
        <section className="grid gap-5 xl:grid-cols-2 2xl:grid-cols-4">
          <ImportCard icon={<Upload className="size-4" />} tone="scolapro-tone-mint" title="Stage learner file" description="Required: first_names, surname, grade_code, class_code. Stable identifiers: admission_number, national_id, birth_certificate_number." inputId="learner-csv" label="Choose learner CSV or XLSX" helper="Maximum 5 MB. CSV or Excel files. Rows are staged before any learner record is created." accept=".csv,.xlsx,.xls" button="Stage and reconcile learners" action={stageLearnerCsv} />
          <ImportCard icon={<UsersRound className="size-4" />} tone="scolapro-tone-sky" title="Stage staff file" description="Required: employee_number, first_name, last_name. Optional: assignment_type, position_title, effective_from." inputId="staff-csv" label="Choose staff CSV or XLSX" helper="Maximum 5 MB. CSV or Excel files. Imported staff receive an effective school assignment even before an account invitation exists." accept=".csv,.xlsx,.xls" button="Stage and reconcile staff" action={stageStaffCsv} />
          <ImportCard icon={<HeartHandshake className="size-4" />} tone="scolapro-tone-mint" title="Stage guardian CSV" description="Required: learner_admission_number, identity_number, first_names, surname. Optional contacts and relationship flags are supported." inputId="guardian-csv" label="Choose guardian CSV" helper="Guardian identity numbers and learner admission numbers drive deterministic matching. A name mismatch on an existing identity requires human confirmation." button="Stage and reconcile guardians" action={stageGuardianCsv} />
...
function ImportCard({ icon, tone, title, description, inputId, label, helper, accept, button, action }: { icon: ReactNode; tone: string; title: string; description: string; inputId: string; label: string; helper: string; accept?: string; button: string; action: (formData: FormData) => void | Promise<void> }) {
  return <div className="flex flex-col bg-surface p-4 shadow-[var(--shadow-xs)] sm:p-5"><div className="flex items-start gap-3"><span className={`${tone} grid size-9 shrink-0 place-items-center rounded-[var(--radius-sm)]`}>{icon}</span><div><h2 className="scolapro-section-title">{title}</h2><p className="scolapro-section-description">{description}</p></div></div><form action={action} className="mt-4 flex flex-1 flex-col"><input id={inputId} name="file" type="file" accept={accept ?? "text/csv,.csv"} className="sr-only" required /><label htmlFor={inputId} className="flex min-h-28 cursor-pointer flex-col items-center justify-center gap-2 rounded-[var(--radius-sm)] border border-dashed border-border bg-surface-muted px-4 text-center hover:bg-brand-soft/40"><FileSpreadsheet className="size-6 text-brand" /><span className="text-xs font-semibold">{label}</span><span className="text-[0.68rem] text-muted-foreground">{helper}</span></label><button type="submit" className="mt-3 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-[var(--radius-sm)] bg-brand px-4 text-sm font-semibold text-white">{button}</button></form></div>;
}

...
export default async function ReportCardsPage() {
  ...
  return <AppShell><section>
    <div className="mb-6"><h1 className="scolapro-page-title text-[clamp(1.25rem,1.08rem+0.45vw,1.65rem)]">Report cards</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">{canManageReports ? "Generate governed term snapshots from approved official results, then certify the exact version used for printing or parent delivery." : "View report-card status for learners within your teaching scope. Generation, certification, publishing and printing remain with School Administration and school management."}</p></div>
    ...
    <ReportCardWorkspace learners={workspace.learners} snapshots={workspace.snapshots} terms={workspace.terms} renderJobs={workspace.renderJobs} documents={workspace.documents} canManageReports={canManageReports} />
  </section></AppShell>;
}

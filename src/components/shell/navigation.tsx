...
const navigation = [
  ...
  { key: "crc_escalations", label: "CRC referrals", href: "/network/crc-escalations", icon: ShieldCheck },
  { key: "imports", label: "Bulk import", href: "/school/imports", icon: FileSpreadsheet },
  { key: "staff", label: "Staff", href: "/staff", icon: UserRound },
  { key: "responsibilities", label: "Responsibilities", href: "/school/responsibilities", icon: ShieldCheck },
  ...
  { key: "sports_houses", label: "Sports / Houses", href: "/school/sports-houses", icon: Trophy },
  ...
  { key: "library", label: "Library / Textbooks", href: "/library", icon: BookOpenText },
  { key: "room_inventory", label: "Room Inventory", href: "/school/room-inventory", icon: Building2 },
...
const enabledKeysByRole: Record<string, readonly string[]> = {
  ...
  school_admin: ["today", "school_directory", "conduct", "sports_houses", "school_invitations", "school_settings", "crc_custody", "setup", "imports", "staff", "responsibilities", "learners", "guardians", "data_corrections", "timetable", "attendance", "late_arrivals", "my_detention", "teaching", "assessment", "report_cards", "library", "room_inventory", "statutory", "contributions", "absence_reviews", "calendar"],
  principal: ["today", "school_directory", "conduct", "sports_houses", "setup", "school_settings", "crc_custody", "staff", "responsibilities", "learners", "guardians", "data_corrections", "timetable", "attendance", "late_arrivals", "my_detention", "teaching", "assessment", "report_cards", "library", "room_inventory", "statutory", "contributions", "absence_reviews", "calendar"],
  deputy_principal: ["today", "school_directory", "conduct", "sports_houses", "school_settings", "crc_custody", "staff", "responsibilities", "learners", "guardians", "data_corrections", "timetable", "attendance", "late_arrivals", "my_detention", "teaching", "assessment", "report_cards", "library", "room_inventory", "statutory", "contributions", "absence_reviews", "calendar"],
  emis_officer: ["today", "statutory", "school_directory"],
  exam_officer: ["today", "dnea_readiness", "school_directory"],
  circuit_officer: ["today", "dnea_readiness", "statutory", "crc_escalations", "school_directory"],
  regional_officer: ["today", "statutory", "crc_escalations", "school_directory"],
  hod: ["today", "school_directory", "conduct", "sports_houses", "staff", "learners", "guardians", "timetable", "attendance", "my_detention", "teaching", "assessment", "report_cards", "calendar"],
  teacher: ["today", "school_directory", "conduct", "sports_houses", "learners", "timetable", "attendance", "my_detention", "teaching", "assessment", "report_cards", "calendar"],
  class_teacher: ["today", "school_directory", "conduct", "sports_houses", "learners", "guardians", "timetable", "attendance", "my_detention", "teaching", "assessment", "report_cards", "contributions", "absence_reviews", "calendar"],
  counsellor: ["today", "school_directory", "conduct", "crc_custody", "learners", "guardians", "absence_reviews", "my_detention", "calendar"],
  learner_support: ["today", "school_directory", "crc_custody", "learners", "calendar"],
  social_worker: ["today", "school_directory", "crc_custody", "learners", "calendar"],
  librarian: ["today", "school_directory", "learners", "library", "my_detention"],
  ltsm: ["today", "school_directory", "library"],
  learner: ["today", "school_directory", "calendar"],
  parent: ["family", "school_directory"],
  board_member: ["today", "school_directory"],
};
...
export function MobileNavigation({ roleKey, roleKeys = [], extraKeys = [], attentionCounts = {} }: { roleKey?: string; roleKeys?: readonly string[]; extraKeys?: readonly string[]; attentionCounts?: NavigationAttentionCounts }) {
  ...
  return <><nav aria-label="Mobile primary" className="fixed inset-x-0 bottom-0 z-[80] w-full max-w-[100vw] border-t border-border-subtle bg-[color:var(--surface)]/96 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1.5 backdrop-blur-xl lg:hidden"><div className="mx-auto grid min-w-0 max-w-xl gap-1" style={{ gridTemplateColumns: `repeat(${primaryItems.length + (showMore ? 1 : 0)}, minmax(0, 1fr))` }}>{primaryItems.map((item) => {
    const Icon = item.icon;
    const active = isActive(pathname, item.href);
    const count = attentionCounts[item.key] ?? 0;

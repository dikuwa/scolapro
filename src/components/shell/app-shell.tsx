...
export async function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const brand = (
    <Link
      href="/"
      className="mb-5 flex min-h-11 items-center gap-3 rounded-[var(--radius-sm)] px-2 py-2 text-[0.95rem] font-semibold tracking-[-0.02em] transition-colors duration-[var(--motion-fast)] hover:bg-surface-muted group-data-[collapsed=true]/sidebar:justify-center group-data-[collapsed=true]/sidebar:px-1"
    >
      <span className="grid size-9 shrink-0 place-items-center rounded-[var(--radius-sm)] bg-brand text-white shadow-[var(--shadow-xs)]">
  ...
  const footer = (
    <div className="flex items-center gap-2 rounded-[var(--radius-sm)] px-2 py-2 group-data-[collapsed=true]/sidebar:justify-center group-data-[collapsed=true]/sidebar:px-1">
    ...
    <header className="sticky top-0 z-30 border-b border-border-subtle bg-[color:var(--surface)]/92 backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 w-full max-w-[var(--content-max)] items-center gap-3 px-4 sm:px-6 lg:px-8">
        <div className="flex min-w-0 items-center gap-3 lg:hidden">
          <span className="grid size-9 shrink-0 place-items-center rounded-[var(--radius-sm)] bg-brand text-white">
            <GraduationCap aria-hidden="true" className="size-5" />
          </span>
        ...
        <div className="ml-auto flex min-w-0 items-center gap-2 px-1.5 py-1">
          <span className="grid size-8 shrink-0 place-items-center rounded-full bg-surface-subtle text-[0.68rem] font-semibold text-foreground">
            {initials(displayName)}
          </span>

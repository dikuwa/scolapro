...
export function LearnerDirectory({ learners, academicOptions = [] }: { learners: LearnerListItem[]; academicOptions?: GradeOption[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    const rowNumberQuery = /^\d+$/.test(needle);
    return learners
      .filter((learner) => {
        ...
        return (rowNumberQuery || !needle || haystack.includes(needle))
          && (status === "all" || learner.status === status)
          && (grade === "all" || learner.grade === grade)
          && (registerClass === "all" || learner.registerClass === registerClass)
          && (sex === "all" || learner.sex === sex);
      })
      .sort((a, b) => {
        const comparison = formatPersonName(a.name).localeCompare(formatPersonName(b.name), undefined, { numeric: true });
        return sortOrder === "asc" ? comparison : -comparison;
      });
  }, [grade, learners, query, registerClass, sex, sortOrder, status]);

  useEffect(() => {
    const needle = query.trim();
    if (!/^\d+$/.test(needle)) return;
    const rowNumber = Number(needle);
    if (rowNumber < 1 || rowNumber > filtered.length) return;
    document.getElementById(`learner-row-${rowNumber}`)?.scrollIntoView({ behavior: "smooth", block: "center" });
  }, [filtered.length, query]);

  const hasFilters = Boolean(query || status !== "current" || grade !== "all" || registerClass !== "all" || sex !== "all" || sortOrder !== "asc");
  function clearFilters() { setQuery(""); setStatus("current"); setGrade("all"); setRegisterClass("all"); setSex("all"); setSortOrder("asc"); }
  ...
  return (
    <>
      <div className="mb-4 grid gap-2 rounded-[var(--radius-md)] bg-surface-muted/55 p-3 xl:grid-cols-[minmax(15rem,1fr)_auto] xl:items-center">
        <label className="scolapro-control-surface flex min-h-10 min-w-0 items-center gap-2 rounded-[var(--radius-sm)] px-3 sm:max-w-lg"><Search aria-hidden="true" className="size-4 shrink-0 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search learner name/admission no. or type a row number…" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground/70" autoComplete="off" />{query ? <button type="button" onClick={() => setQuery("")} aria-label="Clear search" className="grid size-7 place-items-center rounded-[var(--radius-xs)] text-muted-foreground hover:bg-surface-muted hover:text-foreground"><X className="size-3.5" /></button> : null}</label>
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5 xl:min-w-[42rem]"><Picker ariaLabel="Learner status" name="learner-status-filter" value={status} onChange={setStatus} placeholder="Status" options={[{ value: "current", label: "Current" }, { value: "all", label: "All statuses" }]} /><Picker ariaLabel="Filter by grade" name="learner-grade-filter" value={grade} onChange={(value) => { setGrade(value); setRegisterClass("all"); }} placeholder="All grades" options={[{ value: "all", label: "All grades" }, ...grades.map((item) => ({ value: item, label: item }))]} /><Picker ariaLabel="Filter by register class" name="learner-class-filter" value={registerClass} onChange={setRegisterClass} placeholder="All classes" options={[{ value: "all", label: "All classes" }, ...classes.map((item) => ({ value: item, label: item }))]} /><Picker ariaLabel="Filter by gender" name="learner-sex-filter" value={sex} onChange={setSex} placeholder="All genders" options={[{ value: "all", label: "All genders" }, { value: "female", label: "Girls" }, { value: "male", label: "Boys" }, { value: "other", label: "Other" }, { value: "unspecified", label: "Unspecified" }]} /><Picker ariaLabel="Sort learners" name="learner-sort" value={sortOrder} onChange={(value) => setSortOrder(value === "desc" ? "desc" : "asc")} placeholder="A–Z" options={[{ value: "asc", label: "A–Z" }, { value: "desc", label: "Z–A" }]} /></div>
        {hasFilters ? <button type="button" onClick={clearFilters} className="min-h-8 justify-self-start rounded-[var(--radius-xs)] px-2 text-[0.7rem] font-medium text-muted-foreground hover:bg-surface hover:text-foreground xl:col-start-2 xl:justify-self-end">Clear filters</button> : null}
      </div>
      ...
      <section className="overflow-hidden rounded-[var(--radius-md)] border border-border-subtle bg-surface shadow-[var(--shadow-xs)]">
        ...
        {filtered.length ? <div className="max-h-[70vh] overflow-auto">

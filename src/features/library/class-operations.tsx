  ...
  return <div className="space-y-5">
    <section className="rounded-[var(--radius-md)] bg-surface p-4 shadow-[var(--shadow-xs)] sm:p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <Picker label="Grade" value={gradeId} onChange={(value) => { setGradeId(value); setClassId(""); setPairing({}); setConditions({}); }} searchable options={[{ value: "", label: "All grades" }, ...latestGrades.map((grade) => ({ value: grade.id, label: grade.name, helper: grade.code }))]} />
        <Picker label="Class" value={classId} onChange={(value) => { setClassId(value); setPairing({}); setConditions({}); }} searchable options={[{ value: "", label: "Choose class" }, ...visibleClasses.map((item) => ({ value: item.id, label: item.name, helper: item.code }))]} />
        <Picker label="Textbook / title" value={titleId} onChange={(value) => { setTitleId(value); setPairing({}); setConditions({}); }} searchable options={[{ value: "", label: "All titles" }, ...titles.filter((title) => title.status === "active" && (!gradeId || !title.gradeId || title.gradeId === gradeId)).map((title) => ({ value: title.id, label: title.title, helper: [title.subjectCode, title.gradeCode].filter(Boolean).join(" · ") }))]} />
      </div>
    ...
    {classId && titleId ? <section className="grid gap-5 xl:grid-cols-2">
      ...
      <form action={returnAction} className="rounded-[var(--radius-md)] bg-surface p-4 shadow-[var(--shadow-xs)] sm:p-5">
        ...
        {classActiveLoans.length ? <div className="mt-4 space-y-2">{classActiveLoans.map((loan) => { const copy = copyMap.get(loan.copyId); return <div key={loan.id} className="grid gap-2 rounded-[var(--radius-sm)] bg-surface-muted px-3 py-3 sm:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] sm:items-center"><div><p className="text-xs font-semibold">{loan.borrowerName}</p><p className="mt-0.5 text-[0.68rem] text-muted-foreground">{copy ? copyIdentity(copy) : "Tracked copy"}</p></div><Picker label="Return outcome" value={conditions[loan.id] ?? "good"} onChange={(condition) => setConditions((current) => ({ ...current, [loan.id]: condition }))} options={returnConditions} /></div>; })}</div> : <div className="mt-4 rounded-[var(--radius-sm)] border border-dashed border-border p-5 text-center text-sm text-muted-foreground">No active issues for this class and title.</div>}

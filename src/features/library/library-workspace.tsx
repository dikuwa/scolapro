...
import { cacheLibraryCirculationSnapshot, queueLibraryCirculation } from "@/features/library/offline/circulation-queue";
...
function ReturnLoanForm({ loan, title, copy, offlineScope }: { loan: LibraryLoan; title?: LibraryTitle; copy?: LibraryCopy; offlineScope: OfflineScope | null }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await queueLibraryCirculation(offlineScope, {
        action: "return",
        loanId: loan.id,
        returnedCondition: String(form.get("returnedCondition") ?? condition),
        notes: String(form.get("notes") ?? "").trim() || null,
      });
      toast.success("Return saved on this device. It will sync when the connection returns.");
    } catch {
      toast.error("The return could not be stored on this device.");
    }
  }
  return <form action={action} onSubmit={handleSubmit} className="mt-3 grid gap-3 rounded-[var(--radius-sm)] bg-surface-muted/55 p-3 sm:grid-cols-[minmax(0,0.72fr)_minmax(0,1fr)_auto] sm:items-end">
    <input type="hidden" name="loanId" value={loan.id} />
    <Picker label="Return condition" name="returnedCondition" value={condition} onChange={setCondition} options={returnConditions} placeholder="Good" />
  ...
  return <div className="space-y-5">
    ...
    {view === "catalog" ? <section className="rounded-[var(--radius-md)] bg-surface p-4 shadow-[var(--shadow-xs)] sm:p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <Picker label="Subject" value={subjectId} onChange={setSubjectId} searchable options={[{ value: "", label: "All subjects" }, ...subjects.filter((subject) => subject.status === "active").map((subject) => ({ value: subject.id, label: subject.name, helper: subject.code }))]} placeholder="All subjects" />
        <Picker label="Grade" value={gradeId} onChange={setGradeId} searchable options={[{ value: "", label: "All grades" }, ...latestGrades.map((grade) => ({ value: grade.id, label: grade.name, helper: grade.code }))]} placeholder="All grades" />
        <Picker label="Title status" value={catalogStatus} onChange={setCatalogStatus} options={[{ value: "", label: "All title states" }, { value: "active", label: "Active" }, { value: "inactive", label: "Inactive" }, { value: "archived", label: "Archived" }]} placeholder="All title states" />
        <Picker label="Copy availability" value={availability} onChange={setAvailability} options={[{ value: "", label: "All stock states" }, ...["available", "on_loan", "reserved", "repair", "lost", "withdrawn"].map((value) => ({ value, label: pretty(value) }))]} placeholder="All stock states" />
      </div>
    ...
    {view === "circulation" ? <Circulation today={today} titles={titles} copies={copies} loans={loans} borrowers={borrowers} grades={latestGrades} classes={classes} offlineScope={offlineScope} /> : null}
...
function Circulation({ today, titles, copies, loans, borrowers, grades, classes, offlineScope }: { today: string; titles: LibraryTitle[]; copies: LibraryCopy[]; loans: LibraryLoan[]; borrowers: LibraryBorrower[]; grades: LibraryGrade[]; classes: LibraryClass[]; offlineScope: OfflineScope | null }) {
  ...
  async function handleIssueSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("Issue saved on this device. It will sync when the connection returns.");
    } catch {
      toast.error("The issue could not be stored on this device.");
    }
  }

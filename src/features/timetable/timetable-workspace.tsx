        ...
        <div className="grid gap-5 xl:grid-cols-2 xl:items-start">
          <section className="rounded-[var(--radius-md)] bg-surface p-4 shadow-[var(--shadow-xs)] sm:p-5">
            ...
            <form action={offeringAction} className="mt-4 grid gap-3 border-t border-border-subtle pt-4 sm:grid-cols-2 sm:items-end">
              ...
              <Picker label="Subject" name="subjectId" value={offeringSubjectId} onChange={setOfferingSubjectId} placeholder="Choose subject" options={workspace.subjects.map((item) => ({ value: item.id, label: item.name, helper: item.code.toUpperCase() }))} searchable={workspace.subjects.length > 8} searchPlaceholder="Search subject or code" />
          ...
          <section className="rounded-[var(--radius-md)] bg-surface p-4 shadow-[var(--shadow-xs)] sm:p-5">
            ...
            <form action={allocationAction} className="grid gap-3 sm:grid-cols-2 sm:items-end">
              ...
              <Picker label="Register class" name="classId" value={allocationClassId} onChange={setAllocationClassId} placeholder="Choose class" options={allocationClassOptions.map((item) => ({ value: item.id, label: item.name, helper: item.gradeName }))} searchable={allocationClassOptions.length > 8} searchPlaceholder="Search class or grade" />
          ...
          <section className="rounded-[var(--radius-md)] bg-surface p-4 shadow-[var(--shadow-xs)] sm:p-5">
            ...
            <form action={slotAction} className="grid gap-3 sm:grid-cols-2 sm:items-end">
              ...
              <Picker label="Class" name="classId" value={slotClassId} onChange={(value) => { setSlotClassId(value); setSlotAllocationId(""); }} placeholder="Choose class" options={workspace.classes.map((item) => ({ value: item.id, label: item.name, helper: item.gradeName }))} searchable={workspace.classes.length > 8} searchPlaceholder="Search class or grade" />
              ...
              <Picker label="Room" name="roomId" value={slotRoomId} onChange={setSlotRoomId} placeholder="No room" options={[{ value: "", label: "No room" }, ...workspace.rooms.map((item) => ({ value: item.id, label: item.name, helper: [item.code, item.block, item.capacity ? `${item.capacity} seats` : null].filter(Boolean).join(" · ") }))]} searchable={workspace.rooms.length > 8} searchPlaceholder="Search room, block or code" />

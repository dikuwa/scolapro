        ...
        <div className="grid gap-5 xl:grid-cols-2 xl:items-start">
          ...
          <section className="rounded-[var(--radius-md)] bg-surface p-4 shadow-[var(--shadow-xs)] sm:p-5">
            ...
            <form action={slotAction} className="grid gap-3 sm:grid-cols-2 sm:items-end">
              ...
              <Picker label="Room" name="roomId" value={slotRoomId} onChange={setSlotRoomId} placeholder="No room" options={[{ value: "", label: "No room" }, ...workspace.rooms.map((item) => ({ value: item.id, label: item.name, helper: [item.code, item.block, item.capacity ? `${item.capacity} seats` : null].filter(Boolean).join(" · ") }))]} searchable searchPlaceholder="Search room, block or code" />

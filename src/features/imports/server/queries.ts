...
export async function getImportWorkspace(schoolId: string, selectedBatchId?: string) {
  ...
  let rows: { id: string; row_number: number; source_data: Record<string,string>; normalized_data: Record<string,string>; resolution: string; issues: { level?: string; field?: string; message?: string }[] }[] = [];
  if (selectedId) {
    const { data } = await supabase.from("import_rows").select("id,row_number,source_data,normalized_data,resolution,issues").eq("batch_id", selectedId).order("row_number").limit(300);

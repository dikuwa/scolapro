...
import { canHideReportBatch } from "./report-batch-visibility";
...
export function PagedReportCardManagement(props: Props) {
  ...
  const visibleBatches = props.batches.filter((batch) => !hiddenBatchIds.has(batch.id) || !canHideReportBatch(batch));
  const hideableBatches = visibleBatches.filter(canHideReportBatch);
  const [filterGrade, setFilterGrade] = useState(props.filterGradeId);
  ...
  const tableClasses = props.classes.filter((item) => !filterGrade || item.gradeId === filterGrade);
  ...
  return <div className="space-y-5">
    ...
    {mode === "bulk" ? <>
      ...
      {props.batches.length ? <section className="rounded-[var(--radius-md)] bg-surface-muted p-4 sm:p-5"><div className="mb-3"><h2 className="scolapro-section-title">Batch progress</h2><p className="scolapro-section-description">Recent durable report-card jobs and learner-level exceptions. Hiding finished jobs applies to this visit only.</p><div className="mt-3 flex flex-wrap gap-2">{hideableBatches.length ? <form data-confirm-destructive="true" data-confirm-label="hide finished report progress for this visit; all records and artifacts are retained" onSubmit={(event) => { event.preventDefault(); setHiddenBatchIds((current) => new Set([...current, ...hideableBatches.map((batch) => batch.id)])); }}><Button type="submit" variant="neutral" size="sm">Hide finished jobs</Button></form> : null}{hiddenBatchIds.size ? <Button variant="ghost" size="sm" onClick={() => setHiddenBatchIds(new Set())}>Show hidden jobs</Button> : null}</div></div><div className="grid gap-2 lg:grid-cols-2">{visibleBatches.slice(0, 20).map((batch) => <BatchProgress key={batch.id} batch={batch} issues={props.batchIssues} learnerByEnrolment={learnerNameByEnrolment} />)}</div></section> : null}

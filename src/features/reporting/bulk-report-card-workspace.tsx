...
function BatchButton({ operation, learners, academicYear, termNumber, scopeType, scopeLabel, disabled }: {
  ...
  return <form action={action}>
    ...
    {learners.map((learner) => <input key={learner.enrolmentId} type="hidden" name="enrolmentId" value={learner.enrolmentId} />)}
    <button type="submit" disabled={disabled || pending || learners.length === 0} className={`${actionButton} inline-flex min-h-9 items-center gap-1.5 rounded-[var(--radius-xs)] px-3 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-45 ${config.className}`}>

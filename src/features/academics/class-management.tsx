...
export function ClassManagement({ grades, classes }: { grades: Grade[]; classes: RegisterClass[] }) {
  ...
  useEffect(() => {
    ...
    if (gradeState.success) {
      ...
      queueMicrotask(() => setEditingGradeId(null));
          ...
          <div key={grade.id} className="grid gap-3 py-4 first:pt-0 last:pb-0 sm:grid-cols-[minmax(12rem,0.34fr)_1fr] sm:items-start">
            {editingGrade ? (
              <form action={gradeAction} className="grid gap-2 rounded-[var(--radius-sm)] bg-surface-muted p-3">
                ...
                <div className="flex items-center gap-2"><button type="submit" disabled={gradePending} aria-label="Save grade changes" className="inline-flex min-h-8 items-center gap-1.5 rounded-[var(--radius-xs)] bg-brand px-2.5 text-[0.7rem] font-semibold text-white hover:bg-brand-strong disabled:opacity-60">{gradePending ? <Spinner className="size-3.5 text-white" /> : <Save className="size-3.5" />}Save</button><button type="button" onClick={() => setEditingGradeId(null)} className="min-h-8 rounded-[var(--radius-xs)] px-2.5 text-[0.7rem] font-medium text-muted-foreground hover:bg-surface hover:text-foreground">Cancel</button></div>
              </form>
            ) : (
              <div className="flex items-start justify-between gap-2"><div><p className="scolapro-record-title">{grade.name}</p><p className="mt-0.5 text-[0.68rem] font-medium uppercase tracking-[0.05em] text-muted-foreground">{normalizedCode}</p></div><div className="flex shrink-0 gap-0.5"><button type="button" onClick={() => setEditingGradeId(grade.id)} aria-label={`Edit ${grade.name}`} className="grid size-7 place-items-center rounded-[var(--radius-xs)] text-muted-foreground transition hover:bg-surface-muted hover:text-brand-strong"><Pencil className="size-3.5" /></button><button type="button" disabled={deletePending} onClick={() => startDelete(async () => { const formData = new FormData(); formData.set("gradeId", grade.id); const result = await deleteGrade(formData); if (result.success) toast.success(result.message); else toast.error(result.message); })} aria-label={`Delete ${grade.name}`} className="grid size-7 place-items-center rounded-[var(--radius-xs)] text-muted-foreground transition hover:bg-danger-soft hover:text-[color:var(--danger)] disabled:opacity-50"><Trash2 className="size-3.5" /></button></div></div>
            )}

            <div className="space-y-2">
              {gradeClasses.length ? gradeClasses.map((item) => {
                const editing = editingClassId === item.id;
                if (editing) return <form key={item.id} action={classAction} className="grid gap-2 rounded-[var(--radius-sm)] bg-surface-muted p-3 sm:grid-cols-[0.8fr_1fr_1fr_auto] sm:items-end"><input type="hidden" name="classId" value={item.id} /><Picker label="Grade" name="gradeId" value={classGradeId || item.gradeId} onChange={setClassGradeId} placeholder="Choose grade" options={orderedGrades.map((option) => ({ value: option.id, label: option.name }))} /><div><label className="text-xs font-medium" htmlFor={`code-${item.id}`}>Class code</label><input id={`code-${item.id}`} name="classCode" defaultValue={item.code.toUpperCase()} className={`${fieldClass} mt-1.5 uppercase`} /></div><div><label className="text-xs font-medium" htmlFor={`name-${item.id}`}>Display name</label><input id={`name-${item.id}`} name="displayName" defaultValue={item.name} className={`${fieldClass} mt-1.5`} /></div><div className="flex items-center gap-2"><button type="submit" disabled={classPending} aria-label="Save class changes" className="inline-flex min-h-9 items-center gap-1.5 rounded-[var(--radius-sm)] bg-brand px-3 text-[0.7rem] font-semibold text-white hover:bg-brand-strong disabled:opacity-60">{classPending ? <Spinner className="size-3.5 text-white" /> : <Save className="size-3.5" />}Save</button><button type="button" onClick={() => { setEditingClassId(null); setClassGradeId(""); }} className="min-h-9 rounded-[var(--radius-sm)] px-2.5 text-[0.7rem] font-medium text-muted-foreground hover:bg-surface hover:text-foreground">Cancel</button></div></form>;
                return <div key={item.id} className="flex min-h-10 items-center justify-between gap-3 rounded-[var(--radius-sm)] bg-surface-muted px-3 py-2"><div className="min-w-0"><p className="text-xs font-medium">{item.name}</p><p className="mt-0.5 text-[0.68rem] uppercase tracking-[0.04em] text-muted-foreground">{item.code}</p></div><div className="flex shrink-0 items-center gap-1"><button type="button" onClick={() => { setEditingClassId(item.id); setClassGradeId(item.gradeId); }} aria-label={`Edit ${item.name}`} className="grid size-8 place-items-center rounded-[var(--radius-xs)] text-muted-foreground transition hover:bg-surface hover:text-brand-strong"><Pencil className="size-3.5" /></button><button type="button" disabled={deletePending} onClick={() => startDelete(async () => { const formData = new FormData(); formData.set("classId", item.id); const result = await deleteRegisterClass(formData); if (result.success) toast.success(result.message); else toast.error(result.message); })} aria-label={`Delete ${item.name}`} className="grid size-8 place-items-center rounded-[var(--radius-xs)] text-muted-foreground transition hover:bg-danger-soft hover:text-[color:var(--danger)] disabled:opacity-50"><Trash2 className="size-3.5" /></button></div></div>;

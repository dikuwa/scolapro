...
export function OfficialSummary({ summary, date }: { summary: OfficialAttendanceSummary; date: string }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();

  function navigate(params: { date?: string; term?: string }) {
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="grid gap-3 sm:grid-cols-2 lg:min-w-0 lg:flex-1">
            <Picker
              ...
              value={summary.mode}
              onChange={(next) => navigate(next === "term" ? {} : { date })}
              placeholder="Weekly"
              options={[
      ...
      {!summary.classes.length || !summary.teachingDates.length ? (
        ...
        <section className="overflow-hidden rounded-[var(--radius-md)] bg-surface shadow-[var(--shadow-xs)]">
          <div className="border-b border-border-subtle bg-surface-muted/55 px-4 py-4 sm:px-5">
            <h2 className="scolapro-section-title">{summary.mode === "term" ? `Term summary${summary.term ? ` — ${summary.term.displayName}` : ""}` : "Weekly summary"}</h2>

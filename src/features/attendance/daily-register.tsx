        ...
        {!selectedClassId || !classes.length ? <div className="py-10 text-center"><p className="text-sm font-medium">No register classes configured</p></div> : !rows.length ? <div className="py-10 text-center"><p className="text-sm font-medium">No learners in this class</p></div> : (
          <form action={action}>
            ...
            {focusedRow ? <div className="fixed inset-0 z-[140] flex items-end bg-[color:var(--foreground)]/12 backdrop-blur-[1px] sm:hidden" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setFocusedId(null); }}>
              <div role="dialog" aria-modal="true" aria-label={`Attendance for ${focusedRow.name}`} className="max-h-[calc(100dvh-5rem)] w-full overflow-y-auto rounded-t-[var(--radius-lg)] border border-border-subtle bg-surface p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-[var(--shadow-sm)]">
                <div className="sticky top-0 z-10 -mx-1 flex items-start justify-between gap-3 bg-surface px-1 pb-2"><div><p className="scolapro-section-title">{focusedRow.name}</p><p className="scolapro-section-description">{new Intl.DateTimeFormat("en-NA", { weekday: "long", day: "numeric", month: "long" }).format(new Date(`${attendanceDate}T12:00:00`))}</p></div><div className="flex items-center gap-1"><button type="button" onClick={() => setFocusedId(null)} aria-label="Done editing attendance" className="grid size-9 place-items-center rounded-[var(--radius-xs)] bg-success-soft text-[color:var(--success)] hover:brightness-95"><Check className="size-4" strokeWidth={2.6} /></button><button type="button" onClick={() => setFocusedId(null)} aria-label="Close attendance editor" className="grid size-9 place-items-center rounded-[var(--radius-xs)] text-muted-foreground hover:bg-surface-muted"><X className="size-4" /></button></div></div>
                <div className="mt-3 grid grid-cols-4 gap-1.5">{statuses.map((status) => { const Icon = status.icon; const active = focusedRow.status === status.value; return <button key={status.value} type="button" onClick={() => setStatus(focusedRow, status.value)} className={`inline-flex min-h-11 flex-col items-center justify-center gap-1 rounded-[var(--radius-xs)] px-1 py-2 text-[0.65rem] font-semibold ${statusClass(status.value, active)}`}><Icon className="size-4" aria-hidden="true" strokeWidth={2.4} />{status.label}</button>; })}</div>

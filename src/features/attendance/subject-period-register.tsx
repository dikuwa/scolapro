...
export function SubjectPeriodRegister({ roster, attendanceDate }: { roster: SubjectPeriodRoster; attendanceDate: string }) {
  ...
  return <form action={action} className="space-y-4">
    ...
    <section className="bg-surface shadow-[var(--shadow-xs)]">
      ...
      <div className="flex flex-col gap-2 border-t border-border-subtle bg-surface p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4"><p className="text-[0.68rem] text-muted-foreground">{roster.currentSubmissionId?"Saving creates a new lesson-attendance revision.":"Everyone starts present; record only exceptions."}</p><button type="submit" disabled={pending} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[var(--radius-sm)] bg-brand px-4 text-xs font-semibold text-white disabled:opacity-60">{pending?<Spinner className="size-4 text-white"/>:<Save className="size-4"/>}{pending?"Saving…":roster.currentSubmissionId?"Save revision":"Confirm lesson"}</button></div>
    </section>
  </form>;
}

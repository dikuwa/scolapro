...
function keyFor(enrolmentId: string, date: string) {
...
export function WeeklyRegister({ classes, selectedClassId, dates, learners, reasons, submissionIds }: {
  ...
  const payload = dates.map((date) => ({
    ...
    exceptions: rows.flatMap((row) => {
      ...
      return cell && cell.status !== "present"
        ? [{ enrolment_id: row.enrolmentId, status: cell.status, reason_id: cell.reasonId, note: cell.note }]
        : [];
    }),
  }));
  ...
  function activateCell(row: WeeklyLearnerRow, cell: WeeklyCell) {
    if (cell.status === "present") {
      updateCell(row.enrolmentId, cell.date, { status: "absent", reasonId: null, note: null });
    }
    ...
    <div className="space-y-5">
      <section className="rounded-[var(--radius-md)] bg-surface p-4 shadow-[var(--shadow-sm)] sm:p-5">
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          ...
          <div>
            <p className="text-xs font-medium text-muted-foreground lg:text-right">School week</p>
            <div className="mt-1.5 flex items-center gap-1.5">
              <button type="button" disabled={navigationPending} onClick={() => navigateWeek(-1)} aria-label="Previous week" className="grid size-10 place-items-center rounded-[var(--radius-sm)] bg-surface-muted text-muted-foreground hover:bg-brand-soft hover:text-brand-strong disabled:opacity-50"><ChevronLeft className="size-4" /></button>
              <div className="relative min-w-0 flex-1 rounded-[var(--radius-sm)] bg-surface-muted px-3 py-2 text-center text-sm font-medium sm:min-w-48 sm:flex-none">
                {navigationPending ? <span className="absolute inset-0 grid place-items-center"><Spinner className="size-4 text-brand" /></span> : null}
                <span className={navigationPending ? "opacity-0" : ""}>{dates[0] && dates[4] ? `${new Intl.DateTimeFormat("en-NA", { day: "numeric", month: "short" }).format(new Date(`${dates[0]}T12:00:00`))} – ${new Intl.DateTimeFormat("en-NA", { day: "numeric", month: "short", year: "numeric" }).format(new Date(`${dates[4]}T12:00:00`))}` : "School week"}</span>
              </div>
              <button type="button" disabled={navigationPending} onClick={() => navigateWeek(1)} aria-label="Next week" className="grid size-10 place-items-center rounded-[var(--radius-sm)] bg-surface-muted text-muted-foreground hover:bg-brand-soft hover:text-brand-strong disabled:opacity-50"><ChevronRight className="size-4" /></button>
            </div>
          </div>
        </div>
      </section>
      ...
      <form action={action} className="relative">
          ...
          <div className="border-b border-border-subtle bg-surface-muted/55 px-4 py-4 sm:px-5">
            ...
            <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <label className="scolapro-control-surface flex min-h-10 w-full max-w-md items-center gap-2 rounded-[var(--radius-sm)] px-3">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find learner by name or number…" className="min-w-0 flex-1 bg-transparent text-xs outline-none placeholder:text-muted-foreground/70" />
                {query ? <button type="button" onClick={() => setQuery("")} aria-label="Clear search" className="grid size-7 place-items-center text-muted-foreground"><X className="size-3.5" /></button> : null}
              </label>
              <div className="grid grid-cols-3 gap-1 rounded-[var(--radius-sm)] bg-surface p-1 shadow-[var(--shadow-xs)]">
                {(["all", "male", "female"] as SexFilter[]).map((value) => <button key={value} type="button" onClick={() => setSexFilter(value)} className={`min-h-7 rounded-[var(--radius-xs)] px-2.5 text-[0.7rem] font-medium ${sexFilter === value ? "bg-brand-soft text-brand-strong" : "text-muted-foreground hover:text-foreground"}`}>{value === "all" ? "All" : value === "male" ? "Boys" : "Girls"}</button>)}
              </div>
            </div>
          ...
          {!selectedClassId || !learners.length ? <div className="py-10 text-center"><p className="text-sm font-medium">No learners available for this register</p></div> : <>
            <div className="divide-y divide-border-subtle md:hidden">
              {filteredRows.map((row) => {
                const expanded = expandedMobileLearnerId === row.enrolmentId;
                return <div key={row.enrolmentId} className="px-4 py-2.5">
                  <button type="button" onClick={() => setExpandedMobileLearnerId(expanded ? null : row.enrolmentId)} aria-expanded={expanded} className="flex min-h-11 w-full items-center justify-between gap-3 text-left">
                    <div className="min-w-0"><p className="scolapro-record-title truncate">{row.name}</p><p className="text-[0.68rem] text-muted-foreground">{row.admissionNumber ?? "No admission number"}</p></div>
                  ...
                  {expanded ? <div className="mt-2 grid grid-cols-5 gap-1.5 rounded-[var(--radius-sm)] bg-surface-muted p-2">{row.days.map((cell) => {
                    const style = presentation(cell.status);
                    const Icon = style.icon;
                    return <button key={cell.date} type="button" onClick={() => activateCell(row, cell)} aria-label={`${row.name}, ${cell.date}, ${style.label}`} className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-[var(--radius-xs)] ${style.classes}`}><span className="text-[0.62rem] font-medium opacity-75">{new Intl.DateTimeFormat("en-NA", { weekday: "narrow" }).format(new Date(`${cell.date}T12:00:00`))}</span><Icon className="size-4" strokeWidth={2.4} /></button>;
                  })}</div> : null}
                </div>;
              })}
            </div>

            <div className="hidden overflow-x-auto border-b border-border-subtle md:block">
              <div className="min-w-[46rem]">
                <div className="grid grid-cols-[minmax(12rem,1.2fr)_repeat(5,minmax(5.4rem,0.55fr))] bg-surface-muted px-3 py-2 text-[0.68rem] font-medium text-muted-foreground"><span>Learner</span>{dates.map((date) => <span key={date} className="text-center">{new Intl.DateTimeFormat("en-NA", { weekday: "short", day: "numeric" }).format(new Date(`${date}T12:00:00`))}</span>)}</div>
                <div className="divide-y divide-border-subtle">{filteredRows.map((row) => <div key={row.enrolmentId} className="grid grid-cols-[minmax(12rem,1.2fr)_repeat(5,minmax(5.4rem,0.55fr))] items-center px-3 py-2.5"><div className="min-w-0 pr-3"><p className="scolapro-record-title truncate">{row.name}</p><p className="mt-0.5 text-[0.68rem] text-muted-foreground">{row.admissionNumber ?? "No admission number"}</p></div>{row.days.map((cell) => {
                  const style = presentation(cell.status);
                  const Icon = style.icon;
                  return <button key={cell.date} type="button" onClick={() => activateCell(row, cell)} className={`mx-auto grid size-9 place-items-center rounded-[var(--radius-xs)] transition hover:scale-105 ${style.classes}`} data-tooltip={`${style.label} · click to edit`} aria-label={`${row.name}, ${cell.date}, ${style.label}`}><Icon className="size-4" strokeWidth={2.4} /></button>;
                })}</div>)}</div>
              </div>
            </div>
        ...
        {active ? <div className="fixed inset-0 z-[150] grid items-end bg-[color:var(--foreground)]/12 px-0 pb-[calc(5.25rem+env(safe-area-inset-bottom))] backdrop-blur-[1px] sm:place-items-center sm:p-4" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setActiveKey(""); }}>
          <div role="dialog" aria-modal="true" aria-label={`Attendance for ${active.row.name}`} className="max-h-[calc(100dvh-6.25rem)] w-full overflow-y-auto rounded-t-[var(--radius-lg)] border border-border-subtle bg-surface shadow-[var(--shadow-md)] sm:max-h-[88vh] sm:max-w-lg sm:rounded-[var(--radius-md)]">
            <div className="p-4 pb-0 sm:p-5 sm:pb-0">
              ...
              <div className="mt-4 grid grid-cols-3 gap-1.5">{(["absent", "late", "excused"] as const).map((status) => {
                const style = presentation(status);
                const Icon = style.icon;
                const selected = active?.cell.status === status;
                return <button key={status} type="button" onClick={() => setActiveStatus(status)} className={`inline-flex min-h-9 items-center justify-center gap-1 rounded-[var(--radius-xs)] px-2 text-[0.7rem] font-semibold ${selected ? `${style.classes} ring-1 ring-inset ring-current/20` : "bg-surface-muted text-muted-foreground"}`}>{selected ? <Icon className="size-3.5" strokeWidth={2.4} /> : null}{style.label}</button>;
              })}</div>
              <button type="button" onClick={markActivePresent} className="mt-2 inline-flex min-h-8 items-center gap-1.5 rounded-[var(--radius-xs)] bg-success-soft px-2.5 text-[0.7rem] font-semibold text-[color:var(--success)]"><Check className="size-3.5" />Mark present</button>
              ...
              <div className="mt-3 pb-3"><input id={`weekly-evidence-${active.row.enrolmentId}-${active.cell.date}`} name={`evidence-${active.row.enrolmentId}-${active.cell.date}`} type="file" accept="image/jpeg,image/png,image/webp,application/pdf" capture="environment" className="sr-only" onChange={(event) => setEvidenceNames((current) => ({ ...current, [activeKey]: event.target.files?.[0]?.name ?? "" }))} /><label htmlFor={`weekly-evidence-${active.row.enrolmentId}-${active.cell.date}`} className="inline-flex min-h-9 cursor-pointer items-center gap-1.5 rounded-[var(--radius-xs)] bg-surface-muted px-3 text-[0.7rem] font-medium text-muted-foreground hover:text-foreground"><Paperclip className="size-3.5" />{evidenceNames[activeKey] ? "Change evidence" : "Photo / evidence"}</label>{evidenceNames[activeKey] ? <span className="ml-2 break-all text-[0.68rem] text-muted-foreground">{evidenceNames[activeKey]}</span> : null}</div>
            </div>
            <div className="sticky bottom-0 z-10 border-t border-border-subtle bg-[color:var(--surface)]/96 px-4 pb-[max(0.85rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl sm:static sm:flex sm:justify-end sm:bg-surface sm:px-5 sm:pb-5 sm:pt-3 sm:backdrop-blur-none"><button type="button" onClick={() => setActiveKey("")} className="inline-flex min-h-10 w-full items-center justify-center gap-1.5 rounded-[var(--radius-sm)] bg-brand px-3 text-xs font-medium text-white hover:bg-brand-strong sm:w-auto"><Check className="size-3.5" />Done</button></div>
          </div>
        </div> : null}
      </form>
    </div>

...
export function WeeklyRegister({
  classes,
  selectedClassId,
  dates,
  learners,
  ...
  const filteredRows = rows.filter((row) => {
    ...
    const exceptionMatch = !onlyExceptions || row.days.some((day) => day.status !== "present");
    return searchMatch && exceptionMatch;
  });
  ...
  const payload = useMemo(() => dates.map((date) => ({
    date,
  ...
  })), [dates, mutationIds, rows, submissionIds]);
  ...
  function navigateWeek(direction: -1 | 1) {
    if (!dates[0]) return;
    const next = shiftWeek(dates[0], direction);
    const classParam = selectedClassId ? `&class=${encodeURIComponent(selectedClassId)}` : "";
    startNavigation(() => router.replace(`/attendance?view=week&date=${next}${classParam}`, { scroll: false }));
  }
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          ...
          <div>
            ...
            <div className="mt-1.5 flex items-center gap-1.5">
              ...
              <div className="relative min-w-48 rounded-[var(--radius-sm)] bg-surface-muted px-3 py-2 text-center text-sm font-medium">
                ...
                {dates[0] && dates[4] ? `${new Intl.DateTimeFormat("en-NA", { day: "numeric", month: "short" }).format(new Date(`${dates[0]}T12:00:00`))} – ${new Intl.DateTimeFormat("en-NA", { day: "numeric", month: "short", year: "numeric" }).format(new Date(`${dates[4]}T12:00:00`))}` : "School week"}
              </div>
      ...
      <section className="rounded-[var(--radius-md)] bg-surface p-4 shadow-[var(--shadow-xs)] sm:p-5">
        <div className="-mx-4 -mt-4 border-b border-border-subtle bg-surface-muted/55 px-4 py-4 sm:-mx-5 sm:-mt-5 sm:px-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div><h2 className="scolapro-section-title">Weekly register</h2><p className="scolapro-section-description">Monday to Friday only. Everyone starts present; edit exceptions per learner and day, then confirm the week once.</p></div>
            <div className="flex items-center gap-1.5"><button type="button" onClick={() => setOnlyExceptions(false)} className={`min-h-8 rounded-[var(--radius-xs)] px-2.5 text-xs font-medium ${!onlyExceptions ? "bg-surface text-foreground shadow-[var(--shadow-xs)]" : "text-muted-foreground"}`}>All learners</button><button type="button" onClick={() => setOnlyExceptions(true)} className={`min-h-8 rounded-[var(--radius-xs)] px-2.5 text-xs font-medium ${onlyExceptions ? "bg-surface text-foreground shadow-[var(--shadow-xs)]" : "text-muted-foreground"}`}>Exceptions</button></div>
          </div>
          <div className="mt-3 flex max-w-md items-center gap-2 rounded-[var(--radius-sm)] border border-border-subtle bg-surface px-3 py-2"><Search className="size-4 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find learner…" className="min-w-0 flex-1 bg-transparent text-xs outline-none placeholder:text-muted-foreground/65" /></div>
        </div>
          ...
          <form action={action} className="mt-4">
            ...
            <div className="overflow-x-auto rounded-[var(--radius-sm)] border border-border-subtle">
              <div className="min-w-[46rem]">
                <div className="grid grid-cols-[minmax(12rem,1.2fr)_repeat(5,minmax(5.4rem,0.55fr))] bg-surface-muted px-3 py-2 text-[0.68rem] font-medium text-muted-foreground">
                  <span>Learner</span>{dates.map((date) => <span key={date} className="text-center">{new Intl.DateTimeFormat("en-NA", { weekday: "short", day: "numeric" }).format(new Date(`${date}T12:00:00`))}</span>)}
                </div>
                <div className="divide-y divide-border-subtle">{filteredRows.map((row) => <div key={row.enrolmentId} className="grid grid-cols-[minmax(12rem,1.2fr)_repeat(5,minmax(5.4rem,0.55fr))] items-center px-3 py-2.5"><div className="min-w-0 pr-3"><p className="scolapro-record-title truncate">{row.name}</p><p className="mt-0.5 text-[0.68rem] text-muted-foreground">{row.admissionNumber ?? "No admission number"}</p></div>{row.days.map((cell) => { const currentStatus = statuses.find((item) => item.value === cell.status) ?? statuses[0]; const selected = activeKey === keyFor(row.enrolmentId, cell.date); return <button key={cell.date} type="button" onClick={() => setActiveKey(keyFor(row.enrolmentId, cell.date))} className={`mx-auto grid min-h-9 min-w-12 place-items-center rounded-[var(--radius-xs)] px-2 text-xs font-semibold transition ${selected ? "ring-2 ring-[color:var(--brand)]/30" : ""} ${cell.status === "present" ? "bg-success-soft text-[color:var(--success)]" : "bg-warning-soft text-[color:var(--warning)]"}`} title={`${currentStatus.label} · edit`}><span>{currentStatus.short}</span></button>; })}</div>)}</div>
              </div>
            </div>
            ...
            <div className="sticky bottom-[4.5rem] -mx-4 mt-4 flex items-center justify-between gap-3 border-t border-border-subtle bg-[color:var(--surface)]/96 px-4 py-3 backdrop-blur-xl sm:-mx-5 sm:px-5 lg:bottom-0"><p className="text-xs text-muted-foreground">One confirmation creates separate auditable daily register records for Monday–Friday.</p><button type="submit" disabled={pending} className="scolapro-cta inline-flex min-h-10 shrink-0 items-center gap-2 bg-brand px-4 text-sm font-medium text-white hover:bg-brand-strong disabled:opacity-60">{pending ? <Spinner className="size-4 text-white" /> : <Save className="size-4" />}{pending ? "Saving week…" : "Confirm week"}</button></div>
          </form>
        )}
      </section>
    </div>

...
import { Check, ChevronLeft, ChevronRight, Clock3, Paperclip, Save, Search, ShieldCheck, X } from "lucide-react";
      ...
      <form action={action} className="relative">
        ...
        <section className="overflow-hidden rounded-[var(--radius-md)] bg-surface shadow-[var(--shadow-sm)]">
          <div className="border-b border-border-subtle bg-surface-muted/55 px-4 py-4 sm:px-5">
            <div><h2 className="scolapro-section-title">Weekly register</h2><p className="scolapro-section-description">Everyone starts present. Tap a green tick to record an exception, then add context if needed.</p></div>
          ...
          {!selectedClassId || !learners.length ? <div className="py-10 text-center"><p className="text-sm font-medium">No learners available for this register</p></div> : <>
            <div className="divide-y divide-border-subtle md:hidden">
              {filteredRows.map((row) => <div key={row.enrolmentId} className="px-4 py-3"><div className="mb-2"><p className="scolapro-record-title">{row.name}</p><p className="text-[0.68rem] text-muted-foreground">{row.admissionNumber ?? "No admission number"}</p></div><div className="grid grid-cols-5 gap-1.5">{row.days.map((cell) => { const style = presentation(cell.status); const Icon = style.icon; return <button key={cell.date} type="button" onClick={() => activateCell(row, cell)} aria-label={`${row.name}, ${cell.date}, ${style.label}`} className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-[var(--radius-sm)] ${style.classes}`}><span className="text-[0.62rem] font-medium opacity-75">{new Intl.DateTimeFormat("en-NA", { weekday: "narrow" }).format(new Date(`${cell.date}T12:00:00`))}</span><Icon className="size-4" strokeWidth={2.4} /></button>; })}</div></div>)}
            </div>
        ...
        {active ? <div className="fixed inset-0 z-[130] grid items-end bg-[color:var(--foreground)]/12 p-0 backdrop-blur-[1px] sm:place-items-center sm:p-4" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setActiveKey(""); }}>
          <div role="dialog" aria-modal="true" aria-label={`Attendance for ${active.row.name}`} className="max-h-[88vh] w-full overflow-y-auto rounded-t-[var(--radius-lg)] border border-border-subtle bg-surface p-4 shadow-[var(--shadow-md)] sm:max-w-lg sm:rounded-[var(--radius-md)] sm:p-5">

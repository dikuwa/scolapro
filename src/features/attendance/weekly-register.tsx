        ...
        <section className="overflow-hidden rounded-[var(--radius-md)] bg-surface shadow-[var(--shadow-sm)]">
          ...
          {!selectedClassId || !learners.length ? <div className="py-10 text-center"><p className="text-sm font-medium">No learners available for this register</p></div> : <>
            <div className="divide-y divide-border-subtle md:hidden">
              {filteredRows.map((row) => { const expanded = expandedMobileLearnerId === row.enrolmentId; return <div key={row.enrolmentId} className="px-4 py-2.5"><button type="button" onClick={() => setExpandedMobileLearnerId(expanded ? null : row.enrolmentId)} aria-expanded={expanded} className="flex min-h-11 w-full items-center justify-between gap-3 text-left"><div className="min-w-0"><p className="scolapro-record-title truncate">{row.name}</p><p className="text-[0.68rem] text-muted-foreground">{row.admissionNumber ?? "No admission number"}</p></div>{expanded ? <ChevronUp className="size-4 shrink-0 text-muted-foreground" /> : <ChevronDown className="size-4 shrink-0 text-muted-foreground" />}</button>{expanded ? <div className="mt-2 grid grid-cols-5 gap-1.5 rounded-[var(--radius-sm)] bg-surface-muted p-2">{row.days.map((cell) => { const style = presentation(cell.status); const Icon = style.icon; return <button key={cell.date} type="button" onClick={() => activateCell(row, cell)} aria-label={`${row.name}, ${cell.date}, ${style.label}`} className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-[var(--radius-xs)] ${style.classes}`}><span className="text-[0.62rem] font-medium opacity-75">{new Intl.DateTimeFormat("en-NA", { weekday: "narrow" }).format(new Date(`${cell.date}T12:00:00`))}</span><Icon className="size-4" strokeWidth={2.4} /></button>; })}</div> : null}</div>; })}
            </div>
            ...
            <div className="sticky bottom-[4.4rem] z-10 flex flex-col gap-2 bg-[color:var(--surface)]/96 px-4 py-3 backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between sm:px-5 lg:bottom-0"><p className="text-[0.7rem] text-muted-foreground">One confirmation creates separate auditable daily records for Monday–Friday.</p><button type="submit" disabled={pending} className="scolapro-cta inline-flex min-h-10 items-center justify-center gap-2 bg-brand px-4 text-sm font-medium text-white shadow-[var(--shadow-xs)] hover:bg-brand-strong disabled:opacity-60">{pending ? <Spinner className="size-4 text-white" /> : <Save className="size-4" />}{pending ? "Saving week…" : "Confirm week"}</button></div>
          </>}
        </section>

        {active ? <div className="fixed inset-0 z-[150] grid items-end bg-[color:var(--foreground)]/12 p-0 backdrop-blur-[1px] sm:place-items-center sm:p-4" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setActiveKey(""); }}>
          <div role="dialog" aria-modal="true" aria-label={`Attendance for ${active.row.name}`} className="max-h-[88vh] w-full overflow-y-auto rounded-t-[var(--radius-lg)] border border-border-subtle bg-surface p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-[var(--shadow-md)] sm:max-w-lg sm:rounded-[var(--radius-md)] sm:p-5">
            ...
            <div className="mt-4 grid grid-cols-3 gap-1.5">{(["absent", "late", "excused"] as const).map((status) => { const style = presentation(status); const Icon = style.icon; const selected = active?.cell.status === status; return <button key={status} type="button" onClick={() => setActiveStatus(status)} className={`inline-flex min-h-9 items-center justify-center gap-1 rounded-[var(--radius-xs)] px-2 text-[0.7rem] font-semibold ${selected ? `${style.classes} ring-1 ring-inset ring-current/20` : "bg-surface-muted text-muted-foreground"}`}>{selected ? <Icon className="size-3.5" strokeWidth={2.4} /> : null}{style.label}</button>; })}</div>

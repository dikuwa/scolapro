      ...
      <form action={action} className="relative">
        ...
        <section className="overflow-hidden rounded-[var(--radius-md)] bg-surface shadow-[var(--shadow-xs)]">
          <div className="border-b border-border-subtle bg-surface-muted/55 px-4 py-4 sm:px-5">
            <div><h2 className="scolapro-section-title">Weekly register</h2><p className="scolapro-section-description">Everyone starts present. On phones, open a learner to show Monday–Friday. Tap any day to review or change that learner’s status.</p></div>
          ...
          {!selectedClassId || !learners.length ? <div className="py-10 text-center"><p className="text-sm font-medium">No learners available for this register</p></div> : <>
            <div className="max-h-[min(62vh,42rem)] divide-y divide-border-subtle overflow-y-auto overscroll-contain md:hidden">{filteredRows.map((row) => { const expanded = expandedMobileLearnerId === row.enrolmentId; return <div key={row.enrolmentId} className="px-4 py-2.5"><button type="button" onClick={() => setExpandedMobileLearnerId(expanded ? null : row.enrolmentId)} aria-expanded={expanded} className="flex min-h-11 w-full items-center justify-between gap-3 text-left"><span className="flex min-w-0 items-baseline gap-2"><span className="scolapro-record-title min-w-0 truncate">{row.name}</span><span className="shrink-0 text-[0.68rem] font-normal text-muted-foreground">{row.admissionNumber ?? "No admission number"}</span></span>{expanded ? <ChevronUp className="size-4 shrink-0 text-muted-foreground" /> : <ChevronDown className="size-4 shrink-0 text-muted-foreground" />}</button>{expanded ? <div className="mt-2 grid grid-cols-5 gap-1.5 rounded-[var(--radius-sm)] bg-surface-muted p-2">{row.days.map((cell) => { const style = presentation(cell.status); const Icon = style.icon; return <button key={cell.date} type="button" onClick={() => activateCell(row, cell)} aria-label={`${row.name}, ${cell.date}, ${style.label}`} className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-[var(--radius-xs)] ${style.classes}`}><span className="text-[0.62rem] font-medium opacity-75">{new Intl.DateTimeFormat("en-NA", { weekday: "narrow" }).format(new Date(`${cell.date}T12:00:00`))}</span><Icon className="size-4" strokeWidth={2.4} /></button>; })}</div> : null}</div>; })}</div>

            <div className="hidden max-h-[min(62vh,42rem)] overflow-auto overscroll-contain border-b border-border-subtle md:block"><div className="min-w-[46rem]"><div className="sticky top-0 z-10 grid grid-cols-[minmax(12rem,1.2fr)_repeat(5,minmax(5.4rem,0.55fr))] border-b border-border-subtle bg-surface-muted px-3 py-2 text-[0.68rem] font-medium text-muted-foreground"><span>Learner</span>{dates.map((date) => <span key={date} className="text-center">{new Intl.DateTimeFormat("en-NA", { weekday: "short", day: "numeric" }).format(new Date(`${date}T12:00:00`))}</span>)}</div><div className="divide-y divide-border-subtle">{filteredRows.map((row) => <div key={row.enrolmentId} className="grid grid-cols-[minmax(12rem,1.2fr)_repeat(5,minmax(5.4rem,0.55fr))] items-center px-3 py-2.5"><div className="min-w-0 pr-3"><span className="flex min-w-0 items-baseline gap-2"><span className="scolapro-record-title min-w-0 truncate">{row.name}</span><span className="shrink-0 text-[0.68rem] font-normal text-muted-foreground">{row.admissionNumber ?? "No admission number"}</span></span></div>{row.days.map((cell) => { const style = presentation(cell.status); const Icon = style.icon; return <button key={cell.date} type="button" onClick={() => activateCell(row, cell)} className={`mx-auto grid size-9 place-items-center rounded-[var(--radius-xs)] transition hover:scale-105 ${style.classes}`} data-tooltip={`${style.label} · click to edit`} aria-label={`${row.name}, ${cell.date}, ${style.label}`}><Icon className="size-4" strokeWidth={2.4} /></button>; })}</div>)}</div></div></div>
        ...
        {active ? <div className="fixed inset-0 z-[150] flex items-end justify-center bg-[color:var(--foreground)]/12 px-0 pb-[calc(4.75rem+env(safe-area-inset-bottom))] backdrop-blur-[1px] sm:items-center sm:justify-center sm:p-4" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setActiveKey(""); }}>
          <div role="dialog" aria-modal="true" aria-label={`Attendance for ${active.row.name}`} className="max-h-[calc(100dvh-5.75rem)] w-full overflow-y-auto rounded-t-[var(--radius-lg)] border border-border-subtle bg-surface shadow-[var(--shadow-sm)] sm:max-h-[88vh] sm:max-w-lg sm:rounded-[var(--radius-md)]">
            <div className="p-4 sm:p-5">
              ...
              <div className="mt-3 grid grid-cols-4 gap-1.5 sm:ml-auto sm:max-w-md">{weeklyStatuses.map((status) => { const style = presentation(status.value); const Icon = style.icon; const selected = active?.cell.status === status.value; return <button key={status.value} type="button" onClick={() => setActiveStatus(status.value)} aria-pressed={selected} className={`inline-flex min-h-11 items-center justify-center gap-1 rounded-[var(--radius-xs)] px-2 py-2 text-[0.7rem] font-semibold transition ${selected ? `${style.classes} ring-1 ring-inset ring-current/25` : "bg-surface-muted text-muted-foreground hover:bg-surface-subtle hover:text-foreground"}`}>{selected ? <Icon className="size-4" strokeWidth={2.4} /> : null}{status.label}</button>; })}</div>

...
export async function getDailyRegisterWorkspace(
  ...
  const [{ data: classes, error: classError }, { data: reasons, error: reasonError }] = await Promise.all([
    ...
    supabase
      ...
      .select("id,reason_code,display_name,sensitive")
      .eq("audience", "learner")
      .eq("active", true)
      .order("sort_order"),
  ]);
  ...
  const classOptions: AttendanceClassOption[] = (classes ?? []).map((item) => ({
    id: item.id,
    name: item.display_name,
    grade: relation(item.grades)?.display_name ?? "Grade",
  }));

  const reasonsList: AttendanceReasonOption[] = (reasons ?? []).map((item) => ({
    id: item.id,
  ...
  const classId = selectedClassId && classOptions.some((item) => item.id === selectedClassId)
    ? selectedClassId
    : classOptions[0]?.id ?? null;

  if (!classId) {
    return { classes: classOptions, reasons: reasonsList, selectedClassId: null, learners: [] as AttendanceLearnerRow[], currentSubmissionId: null };
  }
  ...
  const [{ data: enrolments, error: enrolmentError }, { data: currentRows, error: currentError }, { data: submission, error: submissionError }] = await Promise.all([
    supabase
      .from("enrolments")
      .select("id,admission_number,learner_id,learners!inner(id,first_names,surname)")
      .eq("school_id", schoolId)
      .eq("register_class_id", classId)
      .eq("academic_year", academicYear)
    ...
    supabase
      ...
      .select("submission_id,enrolment_id,status,reason_id,note")
      .eq("school_id", schoolId)
      .eq("register_class_id", classId)

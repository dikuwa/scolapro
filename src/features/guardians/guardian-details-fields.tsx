  ...
  return <div className="space-y-5">
    ...
    <section>
      ...
      <div className="space-y-2">{contacts.map((contact, index) => <div key={contact.id} className="grid gap-2 rounded-[var(--radius-sm)] bg-surface p-2.5 sm:grid-cols-[8rem_8rem_minmax(0,1fr)_auto] sm:items-center">
        <Picker aria-label={`Contact type ${index + 1}`} name={`contact-type-${contact.id}`} value={contact.type} onChange={(value) => updateContact(contact.id, { type: value as ContactDraft["type"] })} options={contactTypeOptions} />
    ...
    <section>
      ...
      <div className="space-y-3">{addresses.map((address, index) => <div key={address.id} className="rounded-[var(--radius-sm)] bg-surface p-3">
        <div className="grid gap-2 sm:grid-cols-[8rem_minmax(0,1fr)_auto] sm:items-center">
          <Picker aria-label={`Address type ${index + 1}`} name={`address-type-${address.id}`} value={address.type} onChange={(value) => updateAddress(address.id, { type: value as AddressDraft["type"] })} options={addressTypeOptions} />

  ...
  return <div className="space-y-5">
    ...
    <section>
      ...
      <div className="space-y-2">{contacts.map((contact, index) => <div key={contact.id} className="grid gap-2 rounded-[var(--radius-sm)] bg-surface p-2.5 sm:grid-cols-[8rem_8rem_minmax(0,1fr)_auto] sm:items-center">
        ...
        <div className="flex items-center justify-end gap-1"><label className="inline-flex items-center gap-1 text-[0.65rem] text-muted-foreground"><input type="checkbox" checked={contact.primary} onChange={(event) => updateContact(contact.id, { primary: event.target.checked })} />Primary</label>{contacts.length > 1 ? <button type="button" onClick={() => setContacts((current) => current.filter((item) => item.id !== contact.id))} aria-label="Remove contact" className="grid size-8 place-items-center rounded-[var(--radius-xs)] text-muted-foreground hover:bg-danger-soft hover:text-[color:var(--danger)]"><Trash2 className="size-3.5" /></button> : null}</div>
      </div>)}</div>
    </section>
    ...
    <section>
      ...
      <div className="space-y-3">{addresses.map((address, index) => <div key={address.id} className="rounded-[var(--radius-sm)] bg-surface p-3">
        <div className="grid gap-2 sm:grid-cols-[8rem_minmax(0,1fr)_auto] sm:items-center"><select aria-label={`Address type ${index + 1}`} value={address.type} onChange={(event) => updateAddress(address.id, { type: event.target.value as AddressDraft["type"] })} className={fieldClass}><option value="physical">Physical</option><option value="postal">Postal</option><option value="work">Work</option><option value="other">Other</option></select><input aria-label={`Address label ${index + 1}`} value={address.label} onChange={(event) => updateAddress(address.id, { label: event.target.value })} placeholder="Home, postal…" className={fieldClass} /><div className="flex items-center justify-end gap-1"><label className="inline-flex items-center gap-1 text-[0.65rem] text-muted-foreground"><input type="checkbox" checked={address.primary} onChange={(event) => updateAddress(address.id, { primary: event.target.checked })} />Primary</label>{addresses.length > 1 ? <button type="button" onClick={() => setAddresses((current) => current.filter((item) => item.id !== address.id))} aria-label="Remove address" className="grid size-8 place-items-center rounded-[var(--radius-xs)] text-muted-foreground hover:bg-danger-soft hover:text-[color:var(--danger)]"><Trash2 className="size-3.5" /></button> : null}</div></div>

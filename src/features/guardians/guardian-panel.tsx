...
import { addGuardianRelationship, endGuardianRelationship, type GuardianActionState } from "@/features/guardians/server/actions";
import type { LearnerGuardian } from "@/features/guardians/server/queries";

const initialState: GuardianActionState = {};

export function GuardianPanel({ learnerId, guardians }: { learnerId: string; guardians: LearnerGuardian[] }) {
  const [state, action, pending] = useActionState(addGuardianRelationship, initialState);
  ...
  return (
    <section className="bg-surface shadow-[var(--shadow-xs)]">
      ...
      {open ? <form action={action} className="border-t border-border-subtle bg-surface-muted/45 p-4 sm:p-5"><input type="hidden" name="learnerId" value={learnerId} /><div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-medium">First names<input name="firstNames" className="mt-1.5 min-h-10 w-full rounded-[var(--radius-sm)] border border-border-subtle bg-surface-elevated px-3 outline-none" /></label><label className="text-xs font-medium">Surname<input name="surname" className="mt-1.5 min-h-10 w-full rounded-[var(--radius-sm)] border border-border-subtle bg-surface-elevated px-3 outline-none" /></label><label className="text-xs font-medium">Relationship<input name="relationshipType" placeholder="Mother, father, guardian…" className="mt-1.5 min-h-10 w-full rounded-[var(--radius-sm)] border border-border-subtle bg-surface-elevated px-3 outline-none" /></label><label className="text-xs font-medium">Priority<input name="priority" type="number" min="1" max="20" defaultValue="1" className="mt-1.5 min-h-10 w-full rounded-[var(--radius-sm)] border border-border-subtle bg-surface-elevated px-3 outline-none" /></label><label className="text-xs font-medium">Mobile<input name="mobile" inputMode="tel" className="mt-1.5 min-h-10 w-full rounded-[var(--radius-sm)] border border-border-subtle bg-surface-elevated px-3 outline-none" /></label><label className="text-xs font-medium">Email<input name="email" type="email" className="mt-1.5 min-h-10 w-full rounded-[var(--radius-sm)] border border-border-subtle bg-surface-elevated px-3 outline-none" /></label></div><div className="mt-3 flex flex-wrap gap-3 text-[0.7rem]"><label className="inline-flex items-center gap-1.5"><input name="legalGuardian" type="checkbox" />Legal guardian</label><label className="inline-flex items-center gap-1.5"><input name="emergencyContact" type="checkbox" />Emergency contact</label><label className="inline-flex items-center gap-1.5"><input name="pickupAuthorized" type="checkbox" />Pickup authorized</label></div><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setOpen(false)} className="min-h-9 rounded-[var(--radius-sm)] px-3 text-xs text-muted-foreground">Cancel</button><button type="submit" disabled={pending} className="inline-flex min-h-9 items-center gap-1.5 rounded-[var(--radius-sm)] bg-brand px-3 text-xs font-semibold text-white disabled:opacity-60"><ShieldCheck className="size-3.5" />{pending ? "Saving…" : "Link guardian"}</button></div></form> : null}
    </section>
  );
}

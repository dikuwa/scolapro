...
const migration = await read("supabase/migrations/20260919160000_staff_identity_reconciliation.sql");
...
test("staff reconciliation preserves canonical identity and evidence boundaries", () => {
  ...
  assert.match(migration, /existing_auth_link/);
  assert.match(migration, /explicit_admin_confirmation/);

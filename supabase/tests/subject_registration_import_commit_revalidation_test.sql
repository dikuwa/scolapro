...
select lives_ok($$select public.register_learner_subject('fcd30000-0000-4000-8000-000000000001','fcd50000-0000-4000-8000-000000000001','manual')$$,'control registration starts active');
select lives_ok($$select public.create_import_batch('22222222-2222-4222-8222-222222222222','subject_registrations','race-register.csv',null)$$,'register race batch can be created');
...
select lives_ok($$select public.withdraw_learner_subject_registration((select id from public.learner_subject_registrations where enrolment_id='fcd30000-0000-4000-8000-000000000001' and subject_offering_id='fcd50000-0000-4000-8000-000000000001'),'state changed after import review')$$,'authoritative registration may change after reconciliation preview');
select is((public.commit_subject_registration_import_batch(current_setting('qa.race_register_batch')::uuid)->>'updated')::integer,1,'commit revalidates stale register skip and reactivates requested registration');
...
select lives_ok($$select public.withdraw_learner_subject_registration((select id from public.learner_subject_registrations where enrolment_id='fcd30000-0000-4000-8000-000000000001' and subject_offering_id='fcd50000-0000-4000-8000-000000000001'),'prepare withdraw race')$$,'control registration is withdrawn before withdraw preview');
select lives_ok($$select public.create_import_batch('22222222-2222-4222-8222-222222222222','subject_registrations','race-withdraw.csv',null)$$,'withdraw race batch can be created');
...
select lives_ok($$select public.register_learner_subject('fcd30000-0000-4000-8000-000000000001','fcd50000-0000-4000-8000-000000000001','manual')$$,'registration can become active after withdraw reconciliation preview');
select is((public.commit_subject_registration_import_batch(current_setting('qa.race_withdraw_batch')::uuid)->>'updated')::integer,1,'commit revalidates stale withdraw skip and applies requested withdrawal');

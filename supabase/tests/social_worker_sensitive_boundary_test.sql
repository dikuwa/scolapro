...
-- A second school so the "role name alone grants nothing" jurisdiction check is real.
insert into public.schools(id,tenant_id,name,emis_number,region,town)
values(
  '33333333-3333-4333-8333-333333333333',
  '11111111-1111-4111-8111-111111111111',
  'Other Jurisdiction School','SW-OTHER','Khomas','Windhoek'
);
...
-- The seed class 10A has no register teacher; bind the class teacher so the
-- teacher is an authorized observation recorder for the seed learner's class.
...
-- ---------------------------------------------------------------------------
-- Platform administration is not an explicit support role.
-- ---------------------------------------------------------------------------
...
select is(
  app_private.user_has_explicit_support_role(
...
-- ---------------------------------------------------------------------------
-- An authorized social worker manages confidential cases at their own school.
-- These fixtures give the negative read assertions real data to be blocked from.
-- ---------------------------------------------------------------------------
...
select is(
  (select count(*)::integer from public.learner_support_cases where id='fc130000-0000-4000-8000-000000000001'),
  1,
  'authorized custodian can read the assigned confidential case'
);

reset role;

-- ---------------------------------------------------------------------------
-- A teacher contributes an observation but receives no confidential access.
-- ---------------------------------------------------------------------------
...
select is(
  ...
  0,
  'contributing teacher cannot read confidential learner-support cases'
);

select is(
  (select count(*)::integer from public.learner_health_history
   where school_id='22222222-2222-4222-8222-222222222222'),
  0,
  'contributing teacher cannot read restricted health CRC history'
);
...
-- ---------------------------------------------------------------------------
-- Role name alone grants nothing across jurisdiction.
-- ---------------------------------------------------------------------------
...
select is(
  ...
  0,
  'social worker at another school cannot access unrelated school cases'
);
...
select is(
  (select count(*)::integer from public.learner_health_history
   where school_id='22222222-2222-4222-8222-222222222222'),
  0,
  'social worker at another school cannot read restricted health CRC history'
);

reset role;

-- ---------------------------------------------------------------------------
-- Platform administration sees none of the confidential fixtures.
-- ---------------------------------------------------------------------------
select set_config('request.jwt.claim.sub','fc100000-0000-4000-8000-000000000004',true);
set local role authenticated;
...
select is(
  (select count(*)::integer from public.learner_support_cases
   where school_id='22222222-2222-4222-8222-222222222222'),
  0,
  'platform admin sees no confidential learner-support cases'
);

reset role;

-- ---------------------------------------------------------------------------
-- Private support-role helpers stay private from client roles.
-- ---------------------------------------------------------------------------
select ok(

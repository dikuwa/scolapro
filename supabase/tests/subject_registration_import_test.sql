...
values
  ('11111111-1111-4111-8111-111111111111','22222222-2222-4222-8222-222222222222','50000000-0000-4000-8000-000000000001','DEMO-001','imported'),
  ('11111111-1111-4111-8111-111111111111','22222222-2222-4222-8222-222222222222','50000000-0000-4000-8000-000000000002','DEMO-002','imported')
...
select is(public.stage_import_rows((select id from public.import_batches where source_file_name='subject-register.csv'),'[{"row_number":1,"normalized":{"admission_number":"DEMO-001","academic_year":2026,"subject_code":"SREG","action":"register"}},{"row_number":2,"normalized":{"admission_number":"DEMO-002","academic_year":2026,"subject_code":"SREG","action":"register"}}]'::jsonb),2,'two registration rows can be staged');
...
select is(public.stage_import_rows((select id from public.import_batches where source_file_name='subject-repeat.csv'),'[{"row_number":1,"normalized":{"admission_number":"DEMO-001","academic_year":2026,"subject_code":"SREG","action":"register"}},{"row_number":2,"normalized":{"admission_number":"DEMO-002","academic_year":2026,"subject_code":"SREG","action":"register"}}]'::jsonb),2,'repeat rows can be staged');
...
select is(public.stage_import_rows((select id from public.import_batches where source_file_name='subject-withdraw.csv'),'[{"row_number":1,"normalized":{"admission_number":"DEMO-002","academic_year":2026,"subject_code":"SREG","action":"withdraw"}},{"row_number":2,"normalized":{"admission_number":"DEMO-001","academic_year":2026,"subject_code":"UNKNOWN","action":"register"}}]'::jsonb),2,'withdraw and invalid rows can be staged');
...
select is(public.stage_import_rows((select id from public.import_batches where source_file_name='subject-withdraw.csv'),'[{"row_number":2,"normalized":{"admission_number":"DEMO-001","academic_year":2026,"subject_code":"SREG","action":"register"}}]'::jsonb),1,'invalid row can be corrected in staging');
...
select is((select status from public.learner_subject_registrations where enrolment_id='60000000-0000-4000-8000-000000000002' and subject_offering_id='fca20000-0000-4000-8000-000000000001'),'withdrawn','withdraw action preserves identity and records withdrawn state');
select is((select status from public.learner_subject_registrations where enrolment_id='60000000-0000-4000-8000-000000000001' and subject_offering_id='fca20000-0000-4000-8000-000000000001'),'active','unrelated active learner remains active');

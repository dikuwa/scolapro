...
select is((select count(*)::integer from public.guardian_addresses),1,'assigned class teacher sees only guardian addresses for assigned learner scope');
select is((select count(*)::integer from public.guardian_addresses where guardian_id='ff120000-0000-4000-8000-000000000001'),1,'assigned learner guardian address remains visible');
select is((select count(*)::integer from public.guardian_addresses where guardian_id='ff120000-0000-4000-8000-000000000002'),0,'class teacher cannot browse another class guardian address');

select set_config('request.jwt.claim.sub','ff100000-0000-4000-8000-000000000002',true);
select is((select count(*)::integer from public.guardian_addresses),2,'school administration retains school-wide guardian address visibility');

...
select ok(
  not has_function_privilege('anon','public.create_report_card_batch(uuid,integer,integer,text,text,text,uuid[])','EXECUTE'),
  'anonymous users cannot create report-card batches'
);
select ok(
  has_function_privilege('authenticated','public.create_report_card_batch(uuid,integer,integer,text,text,text,uuid[])','EXECUTE'),
  'authenticated management calls the governed batch creation RPC'
);
select ok(
  not has_function_privilege('authenticated','public.process_report_card_batch_items(integer)','EXECUTE'),
  'authenticated clients cannot execute the batch worker directly'
);
select ok(
  has_function_privilege('service_role','public.process_report_card_batch_items(integer)','EXECUTE'),
  'service role owns batch processing execution'
);
select ok(
  not has_table_privilege('authenticated','public.report_card_batches','INSERT'),
  'authenticated clients cannot insert batch headers directly'
);
select ok(
  not has_table_privilege('authenticated','public.report_card_batch_items','UPDATE'),
  'authenticated clients cannot rewrite batch item outcomes'
);
...
select throws_ok(
  ...
  'Permission denied',
  'ordinary teachers cannot start report-card bulk mutations'
);
reset role;
...
select throws_ok(
  ...
  'Every selected learner must have a current enrolment in this school and academic year',
  'a batch cannot include another school enrolment'
);
...
select is(
  (select count(*)::integer from public.report_card_batches where operation='generate' and scope_label='Missing results learner'),
  1,
  'generation batch header is persisted'
);
select is(
  (select total_items from public.report_card_batches where operation='generate' and scope_label='Missing results learner'),
  1,
  'batch records its learner count'
);

select lives_ok(
  $$select public.process_report_card_batch_items(10)$$,
  'service worker processes pending report-card batch items'
);
select is(
  (select status from public.report_card_batch_items i join public.report_card_batches b on b.id=i.batch_id where b.scope_label='Missing results learner'),
  'skipped',
  'learner without approved official results is explicitly skipped rather than failing the batch'
);
select is(
  (select result_code from public.report_card_batch_items i join public.report_card_batches b on b.id=i.batch_id where b.scope_label='Missing results learner'),
  'no_approved_results',
  'skip outcome preserves the actionable missing-results reason'
);
select is(
  (select status from public.report_card_batches where scope_label='Missing results learner'),
  'completed',
  'a fully processed batch with only expected skips completes cleanly'
);
...
insert into public.report_card_snapshots(
  ...
  data_snapshot,status,generated_by_user_id
) values(
...
select public.process_report_card_batch_items(10);
select is(
  (select status from public.report_card_snapshots where id='ed400000-0000-4000-8000-000000000001'),
  'certified',
  'batch worker certifies the exact current draft snapshot using the recorded batch actor'
);

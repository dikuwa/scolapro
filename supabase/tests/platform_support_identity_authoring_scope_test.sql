...
select is((select count(*)::integer from public.staff_members),0,'platform support cannot enumerate raw staff identities through support-safe school access');
select throws_ok(
...
select ok((select count(*) from public.staff_members)>0,'platform admin can still inspect governed raw staff identities');
reset role;

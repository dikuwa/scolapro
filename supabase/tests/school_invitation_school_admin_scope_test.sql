...
-- A second school in the same tenant that the School Admin does NOT manage.
...
-- ---------------------------------------------------------------------------
-- School Admin invitation scope
-- ---------------------------------------------------------------------------

select set_config('request.jwt.claim.role','authenticated',true);
select set_config('request.jwt.claim.sub','fb000000-0000-4000-8000-000000000001',true);
select set_config('request.jwt.claim.email','school-scope-admin@example.test',true);
set local role authenticated;

-- Raw join tokens are returned only at creation; capture them for the accept step.
...
select is(
  (select count(*)::integer from created_invitation),
  1,
  'school admin can invite a social worker into their own school'
);
...
select throws_ok(
  ...
  'Permission denied',
  'school admin cannot invite into another school'
);
...
select throws_ok(
  ...
  'school admin cannot invite platform-level roles'
);
...
select is(
  (select count(*)::integer from public.school_invitations
   where school_id='22222222-2222-4222-8222-222222222223'),
  0,
...
-- ---------------------------------------------------------------------------
-- Social worker invitation acceptance
-- ---------------------------------------------------------------------------

select set_config('request.jwt.claim.role','authenticated',true);
select set_config('request.jwt.claim.sub','fb000000-0000-4000-8000-000000000002',true);
select set_config('request.jwt.claim.email','sw-accept@example.test',true);
set local role authenticated;

select is(
  (select role_key from public.accept_school_invitation(
    (select invitation_token from created_invitation))),
  'social_worker',
  'social worker invitation acceptance grants the social worker school role'
);
...
select is(
  (select ssa.assignment_type
   ...
   limit 1),
  'support',
  'accepted social worker receives a support school placement'
);

reset role;

-- ---------------------------------------------------------------------------
-- Platform Admin onboarding remains separately available
-- ---------------------------------------------------------------------------

select set_config('request.jwt.claim.role','authenticated',true);
select set_config('request.jwt.claim.sub','fb000000-0000-4000-8000-000000000003',true);
select set_config('request.jwt.claim.email','platform-onboard@example.test',true);
set local role authenticated;
...
select is(
  ...
  1,
  'platform admin onboarding of a first school administrator remains available'
);

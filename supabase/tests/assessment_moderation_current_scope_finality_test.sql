...
select is(
  (select count(*)::integer from pg_policies
   where schemaname='public' and tablename='assessment_schemes'
     and policyname like 'academic leaders can manage assessment schemes [%'),
  0,
  'legacy split scheme mutation policies are removed'
);

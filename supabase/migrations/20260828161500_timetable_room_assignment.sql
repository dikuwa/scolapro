...
begin
  ...
  if p_room_id is null then
    ...
    if exists (
      select 1 from public.timetable_slots other
      where other.id<>p_slot_id and other.status='active'
        and other.school_id=v_slot.school_id and other.academic_year=v_slot.academic_year
        and other.cycle_code=v_slot.cycle_code and other.weekday=v_slot.weekday and other.period_id=v_slot.period_id
        and other.room_id=p_room_id
    ) then
      raise exception 'This room is already booked for that cycle, day and period.' using errcode='23505';
    end if;
  ...
  values(v_slot.tenant_id,v_slot.school_id,auth.uid(),'timetable_slot.room_assigned','timetable_slot',p_slot_id,jsonb_build_object('room_id',p_room_id));
end;
$$;

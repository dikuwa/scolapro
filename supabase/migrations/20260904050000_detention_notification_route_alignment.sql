...
-- not by the school-wide late-arrival manager. Normalize those durable notification
-- links onto the self-scoped supervision workspace without rewriting management
-- notifications that merely happen to use /late-arrivals.
...
before insert or update of title,href
on public.notifications
for each row execute function app_private.align_detention_supervision_notification_href();

-- Repair already-created assignment/duty notifications so users do not retain a
-- link to a management route they may not be authorized to open.
...
'Routes durable detention assignment and scheduled-duty notifications to the self-scoped My detention supervision workspace while leaving unrelated late-arrival management notifications unchanged.';

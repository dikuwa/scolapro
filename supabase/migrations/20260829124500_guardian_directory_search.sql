-- Internal school guardian search for operational communication. Results are built from
-- learner links the caller is actually authorized to see, so a teacher assigned to one
-- learner does not discover unrelated sibling/class records merely because the same
-- guardian exists elsewhere in the school.

create or replace function public.search_guardian_directory(
  p_school_id uuid,
...
as $$
  ...
  select gr.guardian_id,gr.guardian_name,gr.primary_mobile,gr.primary_email,gr.linked_learners
...
'Permission-aware internal guardian directory search by guardian name/contact or authorized linked learner/admission number. Returned learner links are filtered to the caller actual operational scope.';
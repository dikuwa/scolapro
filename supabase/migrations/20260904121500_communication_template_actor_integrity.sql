...
begin
  ...
  if not app_private.user_can_manage_communication_templates(new.created_by_user_id,new.school_id,v_on_date) then
...
begin
  ...
  if not app_private.user_can_manage_communication_templates(new.created_by_user_id,v_school_id,v_created_on) then
  ...
  if tg_op = 'UPDATE' then
    if old.approved_by_user_id is not null and new.approved_by_user_id is distinct from old.approved_by_user_id then
  ...
  if new.status = 'approved' then
    ...
    if not app_private.user_can_manage_communication_templates(new.approved_by_user_id,v_school_id,v_approved_on) then
      raise exception 'Communication template version approver is not authorized for school';
    end if;
    if (tg_op = 'INSERT' or old.status is distinct from 'approved') and auth.uid() is not null and new.approved_by_user_id is distinct from auth.uid() then
      raise exception 'Communication template version approver must match authenticated actor';
    end if;
  elsif tg_op = 'INSERT' and (new.approved_by_user_id is not null or new.approved_at is not null) then

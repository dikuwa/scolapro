-- Stream C regression correction: an undated assessment must not treat a stale
-- enrolment with status='current' as operationally current after its effective period ended.
-- Dated assessments continue to evaluate eligibility on assessment_date.
...
begin
  ...
  select tenant_id,school_id,academic_year,register_class_id,learner_id,
         enrolled_from,enrolled_to,status
    into v_enrolment
  ...
  if v_instance.assessment_date is not null then
    if v_enrolment.enrolled_from>v_instance.assessment_date
       or (v_enrolment.enrolled_to is not null
           and v_enrolment.enrolled_to<v_instance.assessment_date) then
      raise exception 'Learner mark scope mismatch: learner was not enrolled in the assessment class on the assessment date';
    end if;
  elsif v_enrolment.status<>'current'
        or v_enrolment.enrolled_from>current_date
        or (v_enrolment.enrolled_to is not null and v_enrolment.enrolled_to<current_date) then
    raise exception 'Learner mark scope mismatch: learner enrolment is not currently effective';
  end if;
...
'Learner mark integrity binds learner/enrolment to the assessment class and year. Dated assessments use assessment-date enrolment; undated assessments require a currently effective enrolment.';
comment on function public.submit_assessment_for_review(uuid,text) is
'Assessment submission validates completeness against assessment-date enrolment when dated and current effective enrolment when undated.';

...
begin
  ...
  with caller_staff as (
    ...
    where o.school_id=v_current_school_id
      and (coalesce(p_include_resolved,false) or o.status in ('pending','carried_forward'))
  )
  select
    s.obligation_id,
    s.school_id,
...
'Self-scoped detention-supervision read model bound to the caller current school using the same active membership ordering as application context; stale placement or another school cannot broaden visibility.';
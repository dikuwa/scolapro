-- PostgreSQL promotes COALESCE(smallint, integer literal) to integer. Keep the
-- canonical guardian RPC call bound to its smallint priority signature.
...
begin
  ...
  loop
    ...
    v_guardian_id := public.upsert_guardian_relationship(
      ...
      coalesce(nullif(v_row.normalized_data->>'priority','')::smallint,1::smallint),
      v_contacts
    );

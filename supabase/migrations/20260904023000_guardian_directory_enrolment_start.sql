-- Operational guardian directory searches must not expose guardian identity/contact data
-- before the linked learner's current-status enrolment is effective at the school.
...
'Permission-aware internal guardian directory search limited to linked learners whose current-status enrolment is effective today.';
comment on function public.search_guardian_directory_page(uuid,text,integer,integer) is
'Permission-aware paged guardian directory limited to linked learners whose current-status enrolment is effective today.';

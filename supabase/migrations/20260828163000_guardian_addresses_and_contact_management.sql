...
create index guardian_addresses_guardian_current_idx
  on public.guardian_addresses(guardian_id, address_type, is_primary)
  where effective_to is null;

create unique index guardian_addresses_one_primary_type_idx
  on public.guardian_addresses(guardian_id, address_type)
...
begin
  ...
  if not exists(select 1 from public.learner_guardians where learner_id=p_learner_id and guardian_id=p_guardian_id and effective_to is null) then
  ...
  update public.guardian_contacts set effective_to=current_date
  where guardian_id=p_guardian_id and effective_to is null;
  ...
  for v_item in select value from jsonb_array_elements(coalesce(p_contacts,'[]'::jsonb)) loop
    ...
    if v_primary then
      update public.guardian_contacts set is_primary=false
      where guardian_id=p_guardian_id and contact_type=v_type and effective_to is null;
    end if;
  ...
  update public.guardian_addresses set effective_to=current_date
  where guardian_id=p_guardian_id and effective_to is null;
  ...
  for v_item in select value from jsonb_array_elements(coalesce(p_addresses,'[]'::jsonb)) loop
    ...
    if v_primary then
      update public.guardian_addresses set is_primary=false
      where guardian_id=p_guardian_id and address_type=v_type and effective_to is null;
    end if;
    insert into public.guardian_addresses(
      tenant_id,guardian_id,address_type,label,address_line_1,address_line_2,suburb_or_locality,
      town_or_city,region,postal_code,country,is_primary,created_by_user_id
    ) values (
      v_learner.tenant_id,p_guardian_id,v_type,nullif(btrim(coalesce(v_item->>'label','')),''),v_line1,
      nullif(btrim(coalesce(v_item->>'line2','')),''),nullif(btrim(coalesce(v_item->>'locality','')),''),
      nullif(btrim(coalesce(v_item->>'town','')),''),nullif(btrim(coalesce(v_item->>'region','')),''),
      nullif(btrim(coalesce(v_item->>'postalCode','')),''),coalesce(nullif(btrim(coalesce(v_item->>'country','')),''),'Namibia'),
      v_primary,auth.uid()
    );
  end loop;

  insert into public.audit_events(tenant_id,actor_user_id,event_type,entity_type,entity_id,metadata)
  values(v_learner.tenant_id,auth.uid(),'guardian.contact_details.replaced','guardian',p_guardian_id,
    jsonb_build_object('learner_id',p_learner_id,'contacts',jsonb_array_length(coalesce(p_contacts,'[]'::jsonb)),'addresses',jsonb_array_length(coalesce(p_addresses,'[]'::jsonb))));
end;
$$;

...
begin
  ...
  -- v_instance.status <> 'open' / status <> 'open'. Returned work is now also
  -- editable after a governed return, while review/verified/locked remain closed.
...
begin
  ...
  if v_instance.assessment_date is not null then
    ...
    where e.school_id=v_instance.school_id
      ...
      and (
        not exists (select 1 from public.learner_subject_registrations any_lsr where any_lsr.enrolment_id=e.id)
        or exists (
          select 1 from public.learner_subject_registrations lsr
  ...
  insert into public.mark_submissions(
    ...
    jsonb_build_object(
      ...
      'eligibility','subject-registration-aware'
    ),

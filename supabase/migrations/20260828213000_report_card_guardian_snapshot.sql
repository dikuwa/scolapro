  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'contacts', coalesce((
        ...
        where gc.guardian_id = gp.id and gc.effective_to is null
      ), '[]'::jsonb),
      'addresses', coalesce((
        select jsonb_agg(jsonb_build_object(
        ...
        ) order by ga.is_primary desc, ga.address_type, ga.created_at)
        from public.guardian_addresses ga
        where ga.guardian_id = gp.id and ga.effective_to is null
      ), '[]'::jsonb)
...
begin
  ...
  values(v_enrol.tenant_id,v_enrol.school_id,auth.uid(),'report_card.snapshot.generated','report_card_snapshot',v_snapshot_id,jsonb_build_object('enrolment_id',v_enrol.id,'term_number',p_term_number,'snapshot_version',v_version,'guardian_count',jsonb_array_length(coalesce(v_guardians,'[]'::jsonb))));

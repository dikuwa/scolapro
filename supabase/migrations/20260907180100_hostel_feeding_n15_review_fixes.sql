-- N15 Control Room review fixes.
-- Adds governed residency closure and corrects school-wide aggregate semantics.
...
begin
  ...
  if not exists(
    ...
    where sm.school_id=v_residency.school_id
      and sm.tenant_id=v_residency.tenant_id
...
revoke all on function public.end_hostel_residency(uuid,date) from public,anon;
grant execute on function public.end_hostel_residency(uuid,date) to authenticated;
...
comment on function public.end_hostel_residency(uuid,date) is 'Governed N15 closure of an open hostel residency. Preserves historical effective dating and records an audit event.';

-- Issue #636: bounded, governed Sports / Houses learner workspace read model.
-- Returns the current school/year learner roster and assignment state in one read,
-- avoiding the enrolments -> learner identity hydration fan-out on the page path.
...
begin
  ...
  -- Sports / Houses never treats Platform Support as a school operator. Keep
  -- Platform Admin as the governed cross-school read path, matching the route.

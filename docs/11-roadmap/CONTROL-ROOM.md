...
### 20 September post-release sequence

Release-hardening Issue #598 is CLOSED with RELEASE GO on deployed main `7ad6aefd1fe505e01240f94cfe6fb12867b0bd86`.

Closed release acceptance:
- #577 — closed; authoritative attendance persistence/idempotency evidence accepted and no PostgreSQL persistence defect reproduced.
- #561 — closed; no source defect, and the unavailable production role matrix is an environment/data limitation rather than release work.
- #562 — closed; no source defect, and production has no learning-resource titles/copies/loans for honest circulation acceptance.
- Offline Phase 1 is integrated: service worker shell, scoped IndexedDB attendance snapshots/queue, explicit sync state, server revalidation/idempotency, logout/context clearing and online-first evidence.
- Vercel functions are pinned to `dub1` alongside Supabase `eu-west-1`.
- #590 remains OPEN only as post-release staff-data governance cleanup. Do not merge identities by name, manually move Auth identities or weaken strong-evidence rules.
...
1. Issue #602 offline-first expansion slices.
2. #590 post-release governed identity cleanup once strong authority/evidence exists.

...
Current disposition: **PRODUCTION RELEASE GO / POST-RELEASE HARDENING ACTIVE**.

Release closure:
- #598 — CLOSED; deployed release candidate accepted.
- #577 — CLOSED; authoritative attendance persistence/idempotency accepted, reported loss not reproduced as a database persistence defect.
- #561 — CLOSED; no implementation defect, live role matrix unavailable without fabricating production memberships.
- #562 — CLOSED; no implementation defect, production has no learning-resource titles/copies/loans for honest circulation UAT.
- #590 — OPEN as post-release data-governance cleanup only; existing strong-evidence reconciliation rules remain authoritative.
- Offline Phase 1 is COMPLETE / INTEGRATED for daily-register attendance: PWA service worker, scoped IndexedDB snapshots/queue, reconnect sync, explicit queue states, server scope revalidation/idempotency and logout/context clearing.
